	...
	BX.ready(function() {
		...
		if(isset($arElement["OFFERS"]) && !empty($arElement["OFFERS"]) && $arSetting["OFFERS_VIEW"]["VALUE"] == "LIST"):
			foreach($arElement["OFFERS"] as $key_off => $arOffer):?>
				props = BX.findChildren(BX("catalog-offer-item-search-<?=$arOffer['ID']?>"), {className: "catalog-item-prop"}, true);
				if(!!props && 0 < props.length) {
					for(i = 0; i < props.length; i++) {
						if(!BX.hasClass(props[i], "empty")) {
							BX("catalog-item-props-mob-search-<?=$arOffer['ID']?>").appendChild(BX.create(
								"DIV",
								{
									props: {
		...
		if(!!qntItems && 0 < qntItems.length) {
			for(i = 0; i < qntItems.length; i++) {					
				qntItems[i].value = BX("quantity_search_<?=$arElement['ITEM_ID']?>").value;
			}
		}
	});
</script>
	...
	<div class="item_image" id="<?=$arItemIDs['PICT']?>">
		...
		/***ITEM_IMAGE***/
			...
			<div class="img">
				...
				<?else:?>
					<img src="<?=SITE_TEMPLATE_PATH?>/images/no-photo.jpg" width="150" height="150" alt="<?=strip_tags($arElement['NAME']);?>" />
				<?endif;?>
			</div>
		<?endif;
		/***ITEM_NAME***/?>
							...
							<div class="catalog-item" id="catalog-offer-item-search-<?=$arOffer['ID']?>">
								<div class="catalog-item-info">							
									...
									if(!empty($arOffer["DISPLAY_PROPERTIES"])):?>
										<div id="catalog-item-props-mob-search-<?=$arOffer['ID']?>" class="catalog-item-props-mob"></div>
									<?endif;
									...
									<div class="buy_more">
										...
										if($arOffer["CAN_BUY"]):
											if($arOffer["ASK_PRICE"]):?>
												<a class="btn_buy apuo" id="ask_price_anch_offer_search_<?=$arOffer['ID']?>" href="javascript:void(0)" rel="nofollow"><i class="fa fa-comment-o"></i><span class="short"><?=$arMessage["CATALOG_ELEMENT_ASK_PRICE_SHORT"]?></span></a>
												<?$properties = false;
												...
												<?$APPLICATION->IncludeComponent("altop:ask.price", "",
													Array(
														"ELEMENT_ID" => "offer_search_".$arOffer["ID"],		
											...
											<?elseif(!$arOffer["ASK_PRICE"]):?>
												<div class="add2basket_block">
													...
													<form action="<?=SITE_DIR?>ajax/add2basket.php" class="add2basket_form">
														<div class="qnt_cont">
															<a href="javascript:void(0)" class="minus" onclick="if (BX('quantity_offer_search_<?=$arOffer["ID"]?>').value > <?=$arOffer["CATALOG_MEASURE_RATIO"]?>) BX('quantity_offer_search_<?=$arOffer["ID"]?>').value = parseFloat(BX('quantity_offer_search_<?=$arOffer["ID"]?>').value)-<?=$arOffer["CATALOG_MEASURE_RATIO"]?>;"><span>-</span></a>
															<input type="text" id="quantity_offer_search_<?=$arOffer['ID']?>" name="quantity" class="quantity" value="<?=$arOffer['CATALOG_MEASURE_RATIO']?>"/>
															<a href="javascript:void(0)" class="plus" onclick="BX('quantity_offer_search_<?=$arOffer["ID"]?>').value = parseFloat(BX('quantity_offer_search_<?=$arOffer["ID"]?>').value)+<?=$arOffer["CATALOG_MEASURE_RATIO"]?>;"><span>+</span></a>
														</div>
														...
														<?if(!empty($arElement["SELECT_PROPS"])):?>
															<input type="hidden" name="SELECT_PROPS" id="select_props_offer_search_<?=$arOffer['ID']?>" value="" />
														<?endif;?>
													...
													<button name="boc_anch" id="boc_anch_offer_search_<?=$arOffer['ID']?>" class="btn_buy boc_anch" value="<?=$arMessage['CATALOG_ELEMENT_BOC']?>"><i class="fa fa-bolt"></i><?=$arMessage['CATALOG_ELEMENT_BOC_SHORT']?></button>
													<?$APPLICATION->IncludeComponent("altop:buy.one.click", "", 
														array(
															"IBLOCK_TYPE" => $arParams["IBLOCK_TYPE"],
															"IBLOCK_ID" => $arParams["IBLOCK_ID"],
															"ELEMENT_ID" => $arOffer["ID"],
															"ELEMENT_CODE" => "offer_search",
															"ELEMENT_PROPS" => $props,
															"SELECT_PROP_DIV" => $arItemIDs["SELECT_PROP_DIV"],
											...
											<?endif;
										elseif(!$arOffer["CAN_BUY"]):?>
											<a class="btn_buy apuo" id="order_anch_offer_search_<?=$arOffer['ID']?>" href="javascript:void(0)" rel="nofollow"><i class="fa fa-clock-o"></i><span class="short"><?=$arMessage["CATALOG_ELEMENT_UNDER_ORDER"]?></span></a>
											<?$properties = false;
											foreach($arOffer["DISPLAY_PROPERTIES"] as $propOffer) {
												$properties[] = $propOffer["NAME"].": ".strip_tags($propOffer["DISPLAY_VALUE"]);
											}
				...
				if(isset($arElement["OFFERS"]) && !empty($arElement["OFFERS"])):
					foreach($arElement["OFFERS"] as $key_off => $arOffer):?>
						<div id="buy_more_search_<?=$arElement['ITEM_ID']?>_<?=$arOffer['ID']?>" class="buy_more <?=$arElement['ITEM_ID']?> hidden">
							<?if($arOffer["CAN_BUY"]):											
								if($arOffer["ASK_PRICE"]):?>
									<a class="btn_buy apuo" id="ask_price_anch_offer_search_<?=$arOffer['ID']?>" href="javascript:void(0)" rel="nofollow"><i class="fa fa-comment-o"></i><span><?=$arMessage["CATALOG_ELEMENT_ASK_PRICE_FULL"]?></span></a>
									<?$properties = false;
									foreach($arOffer["DISPLAY_PROPERTIES"] as $propOffer) {
										$properties[] = $propOffer["NAME"].": ".strip_tags($propOffer["DISPLAY_VALUE"]);
									}
								...
								<?endif;
							elseif(!$arOffer["CAN_BUY"]):?>
								<a class="btn_buy apuo" id="order_anch_offer_search_<?=$arOffer['ID']?>" href="javascript:void(0)" rel="nofollow"><i class="fa fa-clock-o"></i><span><?=$arMessage["CATALOG_ELEMENT_UNDER_ORDER"]?></span></a>
								<?$properties = false;
								foreach($arOffer["DISPLAY_PROPERTIES"] as $propOffer) {
									$properties[] = $propOffer["NAME"].": ".strip_tags($propOffer["DISPLAY_VALUE"]);
								}
					...
					<div class="buy_more">
						<?if($arElement["CAN_BUY"]):?>
							<div class="add2basket_block">
								<form action="<?=SITE_DIR?>ajax/add2basket.php" class="add2basket_form">
									<div class="qnt_cont">
										<a href="javascript:void(0)" class="minus" onclick="if (BX('quantity_select_search_<?=$arElement["ITEM_ID"]?>').value > <?=$arElement["CATALOG_MEASURE_RATIO"]?>) BX('quantity_select_search_<?=$arElement["ITEM_ID"]?>').value = parseFloat(BX('quantity_select_search_<?=$arElement["ITEM_ID"]?>').value)-<?=$arElement["CATALOG_MEASURE_RATIO"]?>;"><span>-</span></a>
										<input type="text" id="quantity_select_search_<?=$arElement['ITEM_ID']?>" name="quantity" class="quantity" value="<?=$arElement['CATALOG_MEASURE_RATIO']?>"/>
										<a href="javascript:void(0)" class="plus" onclick="BX('quantity_select_search_<?=$arElement["ITEM_ID"]?>').value = parseFloat(BX('quantity_select_search_<?=$arElement["ITEM_ID"]?>').value)+<?=$arElement["CATALOG_MEASURE_RATIO"]?>;"><span>+</span></a>
									</div>
									...
									if(!empty($arElement["DISPLAY_PROPERTIES"])):
										foreach($arElement["DISPLAY_PROPERTIES"] as $propElement) {
											$props[] = array(
												"NAME" => $propElement["NAME"],
												"CODE" => $propElement["CODE"],
												"VALUE" => strip_tags($propElement["DISPLAY_VALUE"])
											);
										}
									endif;
									$props = !empty($props) ? strtr(base64_encode(addslashes(gzcompress(serialize($props),9))), '+/=', '-_,') : "";?>
...
<?if($arElement["OFFERS"]):
	$arJSParams = array(
		...
		"PRODUCT" => array(
			...
			"PICT" => is_array($arElement["PICTURE_150"]) ? $arElement["PICTURE_150"] : array("SRC" => SITE_TEMPLATE_PATH."/images/no-photo.jpg", "WIDTH" => 150, "HEIGHT" => 150),
		),		
