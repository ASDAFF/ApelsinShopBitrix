	...
	BX.ready(function() {
		...
		if(isset($arElement["OFFERS"]) && !empty($arElement["OFFERS"]) && $arSetting["OFFERS_VIEW"] == "LIST"):
			foreach($arElement["OFFERS"] as $key_off => $arOffer):?>
				props = BX.findChildren(BX("catalog-offer-item-bigdata-<?=$arOffer['ID']?>"), {className: "catalog-item-prop"}, true);
				if(!!props && 0 < props.length) {
					for(i = 0; i < props.length; i++) {
						if(!BX.hasClass(props[i], "empty")) {
							BX("catalog-item-props-mob-bigdata-<?=$arOffer['ID']?>").appendChild(BX.create(
								"DIV",
								{
									props: {
		...
		if(!!qntItems && 0 < qntItems.length) {
			for(i = 0; i < qntItems.length; i++) {					
				qntItems[i].value = BX("quantity_bigdata_<?=$arElement['ID']?>").value;
			}
		}
	});
</script>
							...
							<div class="catalog-item" id="catalog-offer-item-bigdata-<?=$arOffer['ID']?>">
								<div class="catalog-item-info">							
									...
									if(!empty($arOffer["DISPLAY_PROPERTIES"])):?>
										<div id="catalog-item-props-mob-bigdata-<?=$arOffer['ID']?>" class="catalog-item-props-mob"></div>
									<?endif;
									...
									<div class="buy_more">
										...
										if($arOffer["CAN_BUY"]):
											if($arOffer["ASK_PRICE"]):?>
												<a class="btn_buy apuo" id="ask_price_anch_bigdata_<?=$arOffer['ID']?>" href="javascript:void(0)" rel="nofollow"><i class="fa fa-comment-o"></i><span class="short"><?=$arMessage["CATALOG_ELEMENT_ASK_PRICE_SHORT"]?></span></a>
												<?$properties = false;
											...
											<?elseif(!$arOffer["ASK_PRICE"]):?>
												<div class="add2basket_block">
													...
													<button name="boc_anch" id="boc_anch_bigdata_<?=$arOffer['ID']?>" class="btn_buy boc_anch" value="<?=$arMessage['CATALOG_ELEMENT_BOC']?>"><i class="fa fa-bolt"></i><?=$arMessage['CATALOG_ELEMENT_BOC_SHORT']?></button>
											...
											<?endif;
										elseif(!$arOffer["CAN_BUY"]):?>
											<a class="btn_buy apuo" id="order_anch_bigdata_<?=$arOffer['ID']?>" href="javascript:void(0)" rel="nofollow"><i class="fa fa-clock-o"></i><span class="short"><?=$arMessage["CATALOG_ELEMENT_UNDER_ORDER"]?></span></a>
											<?$properties = false;
											foreach($arOffer["DISPLAY_PROPERTIES"] as $propOffer) {
												$properties[] = $propOffer["NAME"].": ".strip_tags($propOffer["DISPLAY_VALUE"]);
											}
				...
				if(isset($arElement["OFFERS"]) && !empty($arElement["OFFERS"])):
					foreach($arElement["OFFERS"] as $key_off => $arOffer):?>
						<div id="buy_more_bigdata_<?=$arElement['ID']?>_<?=$arOffer['ID']?>" class="buy_more <?=$arElement['ID']?> hidden">
							<?if($arOffer["CAN_BUY"]):											
								if($arOffer["ASK_PRICE"]):?>
									<a class="btn_buy apuo" id="ask_price_anch_bigdata_<?=$arOffer['ID']?>" href="javascript:void(0)" rel="nofollow"><i class="fa fa-comment-o"></i><span><?=$arMessage["CATALOG_ELEMENT_ASK_PRICE_FULL"]?></span></a>
									<?$properties = false;
									foreach($arOffer["DISPLAY_PROPERTIES"] as $propOffer) {
										$properties[] = $propOffer["NAME"].": ".strip_tags($propOffer["DISPLAY_VALUE"]);
									}
								...
								<?endif;
							elseif(!$arOffer["CAN_BUY"]):?>
								<a class="btn_buy apuo" id="order_anch_bigdata_<?=$arOffer['ID']?>" href="javascript:void(0)" rel="nofollow"><i class="fa fa-clock-o"></i><span><?=$arMessage["CATALOG_ELEMENT_UNDER_ORDER"]?></span></a>
								<?$properties = false;
								foreach($arOffer["DISPLAY_PROPERTIES"] as $propOffer) {
									$properties[] = $propOffer["NAME"].": ".strip_tags($propOffer["DISPLAY_VALUE"]);
								}
					...
					<div class="buy_more">
						<?if($arElement["CAN_BUY"]):?>
							<div class="add2basket_block">
								<form action="<?=SITE_DIR?>ajax/add2basket.php" class="add2basket_form">
									<div class="qnt_cont">
										<a href="javascript:void(0)" class="minus" onclick="if (BX('quantity_select_bigdata_<?=$arElement["ID"]?>').value > <?=$arElement["CATALOG_MEASURE_RATIO"]?>) BX('quantity_select_bigdata_<?=$arElement["ID"]?>').value = parseFloat(BX('quantity_select_bigdata_<?=$arElement["ID"]?>').value)-<?=$arElement["CATALOG_MEASURE_RATIO"]?>;"><span>-</span></a>
										<input type="text" id="quantity_select_bigdata_<?=$arElement['ID']?>" name="quantity" class="quantity" value="<?=$arElement['CATALOG_MEASURE_RATIO']?>"/>
										<a href="javascript:void(0)" class="plus" onclick="BX('quantity_select_bigdata_<?=$arElement["ID"]?>').value = parseFloat(BX('quantity_select_bigdata_<?=$arElement["ID"]?>').value)+<?=$arElement["CATALOG_MEASURE_RATIO"]?>;"><span>+</span></a>
									</div>
