								...
								if(isset($arItem["OFFERS"]) && !empty($arItem["OFFERS"])):
									...
									<div class="add2basket_block">
										<form action="<?=$APPLICATION->GetCurPage()?>" class="add2basket_form">
											...
											<button type="button" class="btn_buy" name="add2basket" value="<?=GetMessage('CATALOG_ELEMENT_ADD_TO_CART')?>" onclick="OpenPropsPopupBigdata('<?=$arItemIDs["ID"]?>', '<?=$arItem["ID"]?>'<?=($arSetting["OFFERS_VIEW"] == "LIST" ? ", true" : "");?>);"><i class="fa fa-shopping-cart"></i><span><?=GetMessage("CATALOG_ELEMENT_ADD_TO_CART")?></span></button>
										</form>
									</div>
										...
										<?if($arItem["CAN_BUY"]):
											...
											<?elseif(!$arItem["ASK_PRICE"]):
												...
												<?endif;?>
													...
													<button type="button" class="btn_buy" name="add2basket" value="<?=GetMessage('CATALOG_ELEMENT_ADD_TO_CART')?>"<?=(isset($arItem["SELECT_PROPS"]) && !empty($arItem["SELECT_PROPS"]) ? " onclick=\"OpenPropsPopupBigdata('".$arItemIDs["ID"]."', '".$arItem["ID"]."')\"" : " id='".$arItemIDs["BTN_BUY"]."'");?>><i class="fa fa-shopping-cart"></i><span><?=GetMessage("CATALOG_ELEMENT_ADD_TO_CART")?></span></button>
												</form>
											<?endif;
										elseif(!$arItem["CAN_BUY"]):?>
								...
								if(isset($arItem["OFFERS"]) && !empty($arItem["OFFERS"])):
									if($arItem["TOTAL_OFFERS"]["MIN_PRICE"]["CAN_BUY"]):
										if($arItem["TOTAL_OFFERS"]["MIN_PRICE"]["DISCOUNT_VALUE"] > 0):
											...
											<div class="delay">
												<a href="javascript:void(0)" id="catalog-item-delay-bigdata-min-<?=$arItem['TOTAL_OFFERS']['MIN_PRICE']['ID']?>" class="catalog-item-delay" onclick="return addToDelay('<?=$arItem["TOTAL_OFFERS"]["MIN_PRICE"]["ID"]?>', 'quantity_bigdata_<?=$arItem["ID"]?>', '<?=$props?>', '', 'catalog-item-delay-bigdata-min-<?=$arItem["TOTAL_OFFERS"]["MIN_PRICE"]["ID"]?>', '<?=SITE_DIR?>'<?=(isset($arParams["SHOW_POPUP"]) && $arParams["SHOW_POPUP"] == "N") ? ", true" : ""?>)" title="<?=GetMessage('CATALOG_ELEMENT_ADD_TO_DELAY')?>" rel="nofollow"><i class="fa fa-heart-o"></i><i class="fa fa-check"></i></a>
											</div>
										<?endif;
									endif;
								/***ITEM_DELAY***/
										...
										foreach($arItem["PRICES"] as $code=>$arPrice):
											if($arPrice["MIN_PRICE"] == "Y"):
												if($arPrice["DISCOUNT_VALUE"] > 0):
													...
													<div class="delay">
														<a href="javascript:void(0)" id="catalog-item-delay-bigdata-<?=$arItem['ID']?>" class="catalog-item-delay" onclick="return addToDelay('<?=$arItem["ID"]?>', 'quantity_bigdata_<?=$arItem["ID"]?>', '<?=$props?>', '', 'catalog-item-delay-bigdata-<?=$arItem["ID"]?>', '<?=SITE_DIR?>'<?=(isset($arParams["SHOW_POPUP"]) && $arParams["SHOW_POPUP"] == "N") ? ", true" : ""?>)" title="<?=GetMessage('CATALOG_ELEMENT_ADD_TO_DELAY')?>" rel="nofollow"><i class="fa fa-heart-o"></i><i class="fa fa-check"></i></a>
													</div>
												<?endif;
											endif;
										endforeach;
