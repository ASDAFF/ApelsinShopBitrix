<?
$MESS ['PROFILE_DATA_SAVED'] = "Изменения сохранены";
$MESS ['NAME'] = "Имя";
$MESS ['LAST_NAME'] = "Фамилия";
$MESS ['SECOND_NAME'] = "Отчество";
$MESS ['PERSONAL_PHOTO'] = "Изображение профиля";
$MESS ['NEW_PASSWORD_CONFIRM'] = "Подтверждение пароля";
$MESS ['NEW_PASSWORD_REQ'] = "Новый пароль";
$MESS ['MAIN_SAVE'] = "Сохранить";
$MESS ['MAIN_PSWD'] = "Пароль";
$MESS ['LEGEND_PROFILE'] = "Личные данные";
$MESS ['UF_CARD_NUMBER'] = "Номер карты";
$MESS ['MESSAGE_ERROR'] = "Номер карты не найден или указаны неверные данные о владельце.<br>Обратитесь на стойку информации магазина или позвоните в техническую поддержку для решения данной проблемы.";
$MESS ['MESSAGE_ERROR_1'] = "Этот номер карты уже используется.";
?>